# IC Compiler II Reference Methodology 使用教學
...
執行路徑底下會有 ICC2-RM_*.tar.gz，請輸入下列指令解壓縮。
...
根據啟動環境編輯 Makefile 內容以符合環境設定，使用 which icc2_shell_2023 查詢全路徑後修改
...
- [header_icc2_dp.tcl](#header_icc2_dptcl)
- [header_icc2_pnr.tcl](#header_icc2_pnrtcl)
- [icc2_dp_setup.tcl](#icc2_dp_setuptcl)
- [icc2_pnr_setup.tcl](#icc2_pnr_setuptcl)
...
LOGICAL INPUTS     |VERILOG_NETLIST_FILES       |*.vo        |design 給的                 |
LOGICAL INPUTS     |UPF_FILE          |*.upf        |design 給的                 |
PHYSICAL INPUTS     |TCL_FLOORPLAN_FILE        |floorplan.tcl      |可讀取 DP 結果，位於 outputs_icc2/*write_floorplan/floorplan.tcl    |
PHYSICAL INPUTS     |DEF_SCAN_FILE         |*.def        |design 給的                 |

### header_icc2_dp.tcl

configureRM 不支援本檔案修改，手動修改 set search_path 加入 ./design ./floorplan 路徑
