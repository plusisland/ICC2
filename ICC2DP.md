...
設定完 **design.cfg** 檔案後，執行以下命令會開始拷貝並建立工作環境，其中 **APR/icc2** 請替換成實際工作目錄位置。建議先完善所有設定再執行此命令，避免需要重複設定 pnr 底下各個拷貝的檔案。
...
完成後到 **APR/icc2/ICC2-RM_*** 底下。建立 rm_tech_scripts 捷徑指到 TECH 資料夾。
...
| MCMM SCENARIO/MODE/CORNER SETUP |TCL_MCMM_SETUP_FILE | init_design.mcmm_setup.tcl | 參考 examples/TCL_MCMM_SETUP_FILE.*.tcl 並新建於 rm_icc2_pnr_scripts 裡 |
| MCMM SCENARIO/MODE/CORNER SETUP |TCL_PARASITIC_SETUP_FILE | init_design.parasitic_setup.tcl | 參考 examples/TCL_PARASITIC_SETUP_FILE.tcl 並新建於 rm_icc2_pnr_scripts 裡 |
...
| LOGICAL INPUTS | VERILOG_NETLIST_FILES | *.vo | DP 階段 design 給；PNR 階段可讀取 DP 結果，請設定為 dp/$DESIGN_NAME/outputs_icc2/*write_floorplan/*.v.gz* |
| LOGICAL INPUTS | UPF_FILE | *.upf | DP 階段 design 給；PNR 階段可讀取 DP 結果，請設定為 dp/$DESIGN_NAME/outputs_icc2/*write_floorplan/*.upf |
| PHYSICAL INPUTS | TCL_FLOORPLAN_FILE | floorplan.tcl | DP 階段請自行新建 floorplan.tcl；PNR 階段可讀取 DP 結果，請設定為 dp/$DESIGN_NAME/outputs_icc2/*write_floorplan/floorplan.tcl |
| PHYSICAL INPUTS | DEF_SCAN_FILE | *.def | DP 階段 design 給 |
...
| GENERAL | TCL_CTS_NDR_RULE_FILE | cts_ndr.tcl | 參考 examples/cts_ndr.tcl 並新建於 rm_icc2_pnr_scripts 裡 |
